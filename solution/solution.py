...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            relevant = coverage >= relevance_threshold
        ...
        precision_sum = 0.0
        relevant_seen = 0
        for k, relevant in enumerate(relevant_flags, start=1):
            if relevant:
                relevant_seen += 1
...
class LLMJudge:
    ...
    def _build_prompt(
        ...
        return f"""You are an impartial evaluation judge. Score the following AI
response for each criterion on a scale from 0.0 (worst) to 1.0 (best).
...
{{"scores": {{"<criterion>": <0.0 to 1.0>, ...}}, "reasoning": "<one short sentence>"}}
"""
    ...
    def _parse_response(
        self, response: str, rubric: dict[str, Any]
    ) -> dict[str, Any]:
        scores: dict[str, float] = {
            criterion: 0.5 for criterion in rubric
        }
        ...
        if parsed is not None:
            ...
            for criterion in rubric:
                value = score_block.get(criterion)
                if isinstance(value, (int, float)):
                    scores[criterion] = self._clamp_score(value)
        ...
        return {
            "scores": scores,
            "reasoning": (
                str(reasoning).strip() if reasoning else response.strip()
            ),
        }
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            block = entry.get("scores", {})
            if not isinstance(block, dict):
                continue
            values = [
                float(v) for v in block.values() if isinstance(v, (int, float))
            ]
            if not values:
                continue
        ...
        if len(response_averages) >= 2:
            midpoint = len(response_averages) // 2 or 1
            first_half = response_averages[:midpoint]
            second_half = response_averages[midpoint:]
            positional_bias = (
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if (f < 0.4 and r < 0.4) or (f < 0.4 and c < 0.4) or (r < 0.4 and c < 0.4):
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            sugg = suggestions[index - 1] if index - 1 < len(suggestions) else (suggestions[0] if suggestions else "Review pipeline")
            rows.append(f"| {fid} | {ftype} | {cause} | {sugg} | Open |")
    ...
    def generate_improvement_suggestions(
        ...
        if counts.get("irrelevant", 0) > 0 or counts.get("off_topic", 0) > 0 or any(f.relevance < 0.5 for f in failures):
